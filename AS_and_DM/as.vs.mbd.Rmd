...
#if the fisher results are already in the space, do nothing
...
if(!fisher.mbd.in.as.result.loaded)
{
	...
	for (rown in 1:tests.number) 	
	{
		...
		if (0==MN && 0==MY) next
		#it is already filled with 1 by matrix()
...
```
The first test is quite simple. We take all the AS junctions and we perform exact Fisher test (Tumor,Normal)x(MBD signal presence) for each of them. Then, we apply Bonferroni correction for number of AS junctions.
...
# Are there any AS regions with rank that corresponds (_cis_) to the MBD signal presence in a `r width(as.genes.noodles.ranges)[1]` region around the gene?
### Here, we omit T/N classification. 

Actually, we do the same as in prevuious story, but we look not only for same-junction regulation (self-regulation) but for all same-gene .

```{r prepare cisregulation}
...
#)
#rownames(fdr10.cis.res)=NULL
#fdr5.cis.res.with.annot<-cbind(annot.fdr10.cis.res,fdr10.cis.res)
...
# Are there any AS regions with rank that corresponds (_cis_) to the MBD signal presence in other AS junction of the same gene?
### Here, we omit T/N classification. 
...
as.by.mbd.cisreg.reliable.pval<-wilcoxon.as.cis.reg.by.mbd.result$p.value

```

Here is the table for all cis juntion-interations that pass FDR 5%

```{r tablej cis fdr 5%, echo=TRUE}
...
```{r DM for as.by.mbd.reliable.bonf, echo=TRUE }
...
```{r AS for as.by.mbd.reliable.bonf, echo=TRUE }
...
```{r SELFREG for as.by.mbd.reliable.bonf, echo=TRUE }
