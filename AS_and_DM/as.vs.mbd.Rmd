...
fdr5.cis.noodle.res<-kendall.as.cis.reg.by.noodle.mbd.result[which(p.adjust(kendall.as.cis.reg.by.noodle.mbd.result$p.value,method = 'BH')<0.1),]
knitr::kable(fdr5.cis.noodle.res,row.names=TRUE)
```

Here is the list of the genes that carry pairs of junctions with $AS(Junction)$ ~ $Meth(Noodle)$ that pass FDR 10% (for pairs, not for genes).

```{r genes list AS vs noodle M, echo=FALSE}

set<-p.adjust(kendall.as.cis.reg.by.noodle.mbd.result[,'p.value'],method="BH")<0.1
