...
Here is the list of the genes that carry pairs of junctions with $AS(Junction1)$ ~ $Meth(Junction2)$ that pass FDR 5% (for pairs, not for genes). We do not tell $Junction1$ == $Junction2$ from $Junction1$ != $Junction2$.

```{r genes list AS vs M, echo=FALSE}

set<-p.adjust(kendall.as.cis.reg.by.mbd.result[,'p.value'],method="BH")<0.05
...
colnames(result)<-c('gene','as region','dm region','p.value','tau')
knitr::kable(result,row.names=FALSE)
```
...
```{r table cis moodle fdr 5%}
fdr5.cis.noodle.res<-kendall.as.cis.reg.by.noodle.mbd.result[which(p.adjust(kendall.as.cis.reg.by.noodle.mbd.result$p.value,method = 'BH')<0.05),]
knitr::kable(fdr5.cis.noodle.res,row.names=TRUE)
```

Here is the list of the genes that carry pairs of junctions with $AS(Junction)$ ~ $Meth(a noodle in gene)$ that pass FDR 5% (for pairs, not for genes).

```{r genes list AS vs noodle M, echo=FALSE}

set<-p.adjust(kendall.as.cis.reg.by.noodle.mbd.result[,'p.value'],method="BH")<0.05
...
result<-cbind(resultnames,resultstat)
colnames(result)<-c('gene','as region','dm noodle','p.value','tau')
knitr::kable(result,row.names=FALSE)
```
