...
if(!kendall.as.cis.reg.by.mbd.result.loaded)
{
	...
	kendall.as.cis.reg.by.mbd.result.mat<-matrix(nrow=0,ncol=3)
	...
	for (gene in genes)
	{
		...
		the.gene.junctions<-grep(gene,names(as.junction.ranges))
		l<-length(the.gene.junctions)
		the.gene.result.mat<-matrix(c(1,0),nrow=l^2,ncol=3,byrow=TRUE)
		colnames(the.gene.result.mat)<-c('p.value','','exp inv#') 
		for (i in 1:l)
		{
			...
			for (k in 1:l) 	
			{
				...
				the.gene.result.mat[index,2]<-ken$statistic
			}
		}
		#in expand.grid, the second var is outer (slow), so first is k (as)
		rownames(the.gene.result.mat)<-
