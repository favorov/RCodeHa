...
if(!as.and.dm.tables.loaded)
{
	...
	as.junction.ranges<-unlist(GRangesList(lapply(junctionRanks,function(junctions.of.the.gene) {
		colnames(junctions.of.the.gene)<-splice.bed.ids
		juncRanges.gene <- as.data.frame(strsplit(sub('-',':',rownames(junctions.of.the.gene)),':'),stringsAsFactors = FALSE)
		juncRanges.gene <- data.frame(t(juncRanges.gene),stringsAsFactors=FALSE)
		rownames(juncRanges.gene)<-rownames(junctions.of.the.gene)
		colnames(juncRanges.gene)<-c('chr','start','end')
		juncRanges.gene<-cbind(juncRanges.gene,junctions.of.the.gene)
	...
	#so, we prepared a junction GRanges object... Now, differential.coverage()
...
if(!noodles.as.genes.tables.loaded)
{
	...
	as.genes.noodles.ranges<-unlist(GRangesList(lapply(as.genes.symbols,function(gene) {
		...
		gene.noodles<-data.frame('chr'=gene.chr,'start'=seq(noodstart,noodend-noodlen,noodlen),'end'=seq(noodstart+noodlen,noodend,noodlen))
